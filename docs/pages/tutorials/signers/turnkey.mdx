...
import { TurnkeyClient } from "@turnkey/http";
import { createAccount } from "@turnkey/viem";
import { createNexusClient, NexusClient } from "@biconomy/sdk";  // [!code focus] 
...
const main = async () => {
    ...
    const turnkeyAccount = await createAccount({
     ...
     const smartAccountOwner = createWalletClient({  // [!code focus:14] 
    ...
    const nexusClient: NexusClient = await createNexusClient({
        chain: baseSepolia,
        signer: smartAccountOwner.account,
        transport: http("https://sepolia.base.org"),
